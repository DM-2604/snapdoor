...
export default function DashboardPage() {
  ...
  const recentOrders: any[] = []; // Endpoints not yet available for orders
  const topProducts = productsData?.items ?? []; // Using real products if available
  const stats = [
    { label: 'Total Revenue', value: '—', change: '0%', up: true, icon: TrendingUp, color: 'emerald' },
    { label: 'Orders Today', value: '0', change: '0', up: true, icon: ShoppingBag, color: 'blue' },
    { label: 'Active Products', value: productsData?.total?.toString() ?? '0', change: '0', up: true, icon: Package, color: 'amber' },
    { label: 'New Customers', value: '0', change: '0%', up: true, icon: Users, color: 'purple' },
  ];
              ...
              topProducts.map((p: any, idx: number) => (
                <div key={idx} className="flex items-center gap-4 p-3.5 bg-neutral-50 dark:bg-neutral-800/40 rounded-xl hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors border border-transparent hover:border-neutral-200 dark:hover:border-neutral-700">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs font-medium text-neutral-500 mt-0.5">₹{p.basePrice ?? p.sellingPrice ?? 'N/A'}</p>
                  </div>
